...
class C_contacts extends API_Controller {
    public $perpage = 10;
    ...
    public function count_cn(){
        $count = $this->m_contacts->count($this->input->post("brgy_name"));
        if(!empty($count)) 
        {
            $count = count($count);
        }
    ...
    public function retrieveContacts() {
        ...
        $current_page = $ppage;
        $result = $this->m_contacts->retrieveContacts($this->input->post("brgy_name"),$this->perpage,$current_page);
        if ($result){
            $this->responseData($result);
        } else {
            $this->responseError(2, "No Result");
    ...
    public function deleteContact() {
        ...
        $result = $this->m_contacts->deleteContact(
        ...
        if($num_error == "1") {
            ...
            for($x=0;$x<count($temp);$x++){  
                ...
                if (in_array('tmp', $array) ) {
                    $result1 = $this->m_contacts->addContact($temp[$x]["value"], $addBrgyId);
                    $addcn[$z] = array($result1 => $temp[$x]["value"]); 
                    $z++;
                }
                else{
                    $result1 = $this->m_contacts->editContact($temp[$x]["value"], $temp[$x]["id"]);
