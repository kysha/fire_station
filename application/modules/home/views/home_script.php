    ...
    $(document).ready(function () {
        ...
        *********************************************/
        ...
        $("#accordion").on("click", ".getTime", function () {
            ...
            if((d.months()>0 || d.years()>0) || (d.months()<0  || d.years()<0)){
                error_resp = "yearmonth";
                $("#"+panelId).find(".response").removeClass("hidden").text("Response Time reached more than a month/year.");
            }else if(d.days()>=1){
                error_resp = d.days();
                $("#"+panelId).find(".response").removeClass("hidden").text("Response Time reached "+error_resp+" day/s.");
            }else if((d.hours()==0 && d.minutes()==0 && d.seconds()==0) || d.days()<0 ||
                ((d.hours()<0 || d.minutes()<0 || d.seconds()<0) && d.days()>=0)){
                error_resp="error";
                $("#"+panelId).find(".response").removeClass("hidden").text("Check Notification and Arrival Time.");
            }else{
                error_resp="";
                $("#"+panelId).find(".response").addClass("hidden");
            }
        ...
        function showmodal(modal_toshow){
            console.log(modal_toshow);
            if(modal_toshow == "submit_success"){
                $('#submitted').on("show.bs.modal",function(){
                    $(this).find("#deleting_modal").hide();
                    $(this).find('#success_header').empty().addClass('modal-header modal-header-success').show();
                    $(this).find('#success_header').empty().append('<a href="#" data-dismiss="modal" aria-hidden="true" class="close">×</a><h1><i class="glyphicon glyphicon-thumbs-up"></i> Fire Information</h1>');
                    $(this).find('#success_body').empty().append('<p><b>Fire Information Created Successfully!</b></p><p>To Edit go to Logs.</p>').show();
                    $(this).find('#success_footer').empty().append('<button type="submit" data-dismiss="modal" class="btn btn-success btn-md delBTN" id="delCancel"> <span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Ok </button>').show();
                }).modal('show');
            }
        }
        /********************************************
        ...
        *********************************************/
        ...
        function displayTime(){
            var changeDateTime= moment().format('YYYY-MM-DD HH:mm:ss:SSS');
            moment().startOf('hour');
            moment().minutes(0).seconds(0).milliseconds(0);
            console.log("Time: "+changeDateTime);            
        }
        ...
        function deleteYes(container){
            $('#submitted').on("show.bs.modal",function(){
                    $(this).find("#deleting_modal").show();
                    $(this).find('#success_header').empty().hide();
                    $(this).find('#success_body').empty().hide();
                    $(this).find('#success_footer').empty().hide();
                }).modal('show');
            $('#submitted').on("click", "#btn_no", function(response){
                $('#submitted').modal('hide');
            });
            $('#submitted').on("click", "#btn_yes", function(response){
        ...
        *********************************************/
        ...
        function getStationInfo(){
            ...
            var request = $.post("<?=base_url('api/c_substation/retrieveContacts')?>",object_sub);
            //console.log(request);
            request.done(function(response){
        ...
        function checkSubstation(){
            ...
            while(1){
                var contentAdd=$("#"+CurrentId).find("#address").val();
                if (contentAdd=="") {
//                    console.log("empty");
                }
                else{
