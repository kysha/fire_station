...
function buttonchoice(select_page){
    ...
    listSubstation_after(contact_object);
}
...
function listSubstation_after(contact_object){
    $('.paging').empty();
    var count_users = $.post("<?=base_url('api/c_substation/count_cn')?>", contact_object);
    ...
    request.done(function (response) {
        ...
        if(!data["error"].length){
            ...
            $('#table_substation_error').hide();
        }
        else{ 
            $('#table_substation_error').show(); 
